...
function buildPrompt(threshold, mode) {
  ...
  return `Analyze this ${ctx} for AI-generated content.

YOU MUST RESPOND WITH ONLY A JSON OBJECT. No text before or after. No markdown. No code fences. Start with { and end with }.

Required format:
{"overall_score":INTEGER_0_TO_100,"flagged_lines":[{"text":"EXACT_SENTENCE_FROM_DOC","probability":INTEGER_${threshold}_TO_100,"reason":"SHORT_REASON_MAX_8_WORDS"}],"summary":"TWO_SENTENCE_ASSESSMENT"}

Rules:
- overall_score: 0-100 integer (probability whole doc is AI-written)
- flagged_lines: sentences with AI probability >= ${threshold}%. Max 8 lines. If none found, use empty array []
- Each flagged line: copy EXACT text from document, give probability ${threshold}-100, give 3-5 word reason
- AI signals: "furthermore"/"moreover"/"in conclusion", uniform sentence length, perfect structure, no typos, passive voice overuse, generic transitions
- summary: sentence 1 = overall verdict, sentence 2 = recommendation

IMPORTANT: Even if the document is clearly AI-written, you MUST find and list the specific flagged lines. Do not return empty flagged_lines if overall_score > 50.`;
}
