...
// cvBase64/cvMimeType = CV PDF attachment
// jdBase64/jdMimeType = JD PDF attachment (optional — send when JD is a PDF file)
async function callGemini(prompt, cvBase64, cvMimeType, jdBase64, jdMimeType) {
  for (let attempt = 0; attempt < Math.max(KEYS.length, 1); attempt++) {
    const key = getNextKey();
    if (!key) throw new Error('No API keys configured');

    // Build parts — include all available file attachments before the text prompt
    const parts = [];
    if (jdBase64 && jdMimeType) {
      // Send JD PDF first so Gemini reads requirements before scoring the CV
      parts.push({ inline_data: { mime_type: jdMimeType, data: jdBase64 } });
    }
    if (cvBase64 && cvMimeType) {
      parts.push({ inline_data: { mime_type: cvMimeType, data: cvBase64 } });
    }
...
function buildCVPrompt(jd, cvText, cvName, cvIsBase64, jdIsBase64) {
  const cvSection = cvIsBase64
    ? `CANDIDATE CV (${cvName}): [CV PDF is attached as the second document above — read the full CV carefully]`
    : `CANDIDATE CV (${cvName}):\n${cvText.slice(0, 2800)}`;

  const jdSection = jdIsBase64
    ? `JOB DESCRIPTION: [JD PDF is attached as the first document above — read all pages of the JD carefully to extract every requirement]`
    : `JOB DESCRIPTION:\n${jd.slice(0, 1500)}`;

  return `You are a strict senior HR recruiter scoring a CV against a job description.
...
Required schema:
{
  ...
  "matched_attributes": [
    { "attribute": "exact JD requirement", "evidence": "exact quote or phrase from CV proving this", "score": integer 1-10 }
  ],
  "missing": ["JD requirement clearly absent from CV"],
  "strengths": "one sentence about strongest qualification",
  "concern": "one sentence about biggest gap (or empty string if none)",
  "summary": "two sentences: recommendation + suggested next action"
}

SCORING RULES — apply strictly:
1. INDUSTRY FIT is the most important factor. If the JD specifies a sector (e.g. FMCG, Supply Chain, Manufacturing, Healthcare) and the candidate has NO experience in that sector, deduct 30-40 points immediately regardless of other skills.
2. EXPERIENCE YEARS — if the JD requires e.g. 8-15 years and the candidate has 4 years, that is a significant gap. Deduct 10-15 points.
3. DOMAIN SKILLS — only count skills that appear explicitly in the CV with evidence. Do not infer or assume.
4. GENERIC PM/MANAGEMENT skills (project planning, stakeholder management, risk registers) score low unless the JD is generic. Sector-specific skills score high.
5. match_score must reflect ACTUAL fit: 85-100=exceptional match on industry+skills+experience, 70-84=strong, 55-69=moderate, 40-54=weak, below 40=poor or wrong industry.
6. DO NOT default to 50 or 95. Score honestly based on evidence in the CV.

matched_attributes: list every JD requirement present in the CV with specific evidence. Max 10.
missing: list every JD requirement clearly absent. Max 6. Always include industry/domain gaps if present.
  ...
  for (const cv of cvs.slice(0, 5)) {
    ...
    try {
      ...
      const raw = await callGemini(
        ...
        hasJDFile ? jdBase64 : null,
        hasJDFile ? jdMimeType : null
      );
